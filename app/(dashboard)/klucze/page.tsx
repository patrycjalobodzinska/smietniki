...
export default function KeysPage() {
  ...
  // The lokal (podmiotowa) layer is full-mode only; KM1 stays read-only and
  // never touches /v1/units.
  const { data: units } = useKeyUnits(fullMode);
      ...
      ? [
          {
            ...
            header: "Lokal",
            cell: (k: AccessKey) => {
              const label = unitLabels.get(k.unitId);
              return label ? (
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      ...
      {fullMode && <IssueKeyDialog open={issueOpen} onClose={() => setIssueOpen(false)} />}
    </div>
  );
}
