...
import { TriageVariant } from "./variants/triage";
import { MapSplitVariant } from "./variants/map-split";
import { PulseVariant } from "./variants/pulse";
import { CardsVariant } from "./variants/cards";

/**
 * Powierzchnia prototypowa - alternatywy dla tabeli na liście altanek.
 * Odizolowana: nic produkcyjnego stąd nie importuje. Do usunięcia po wyborze.
 */

const VARIANTS = [
  { name: "Pilność", render: () => <TriageVariant /> },
  { name: "Mapa", render: () => <MapSplitVariant /> },
  { name: "Puls", render: () => <PulseVariant /> },
  { name: "Kafle", render: () => <CardsVariant /> },
];
  ...
  return (
    <div className="min-h-dvh bg-background px-6 py-8 pb-28">
      ...
      <header className="mx-auto mb-6 flex max-w-[1400px] items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Altanki</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            12 lokalizacji · prototyp widoku listy ({VARIANTS[current].name})
          </p>
        </div>
        <button
          onClick={toggleTheme}
